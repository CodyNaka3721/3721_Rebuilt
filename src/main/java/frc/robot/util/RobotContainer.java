...
public class RobotContainer {
    ...
    private double MaxSpeed = 0.8 * TunerConstants.kSpeedAt12Volts.in(MetersPerSecond); // kSpeedAt12Volts desired top
                                                                                        // speed Pretty sure this is it
    ...
    private final CommandXboxController BCON = new CommandXboxController(1);
    ...
    public RobotContainer() {
        ...
        NamedCommands.registerCommand("intakePivot107", superstructure.setIntakePivotAngle(Degrees.of(150))); // Name :
                                                                                                              // intakePivot107
        NamedCommands.registerCommand("intakePivot0", superstructure.setIntakePivotAngle(Degrees.of(1))); // Name :
                                                                                                          // intakePivot0
        ...
        NamedCommands.registerCommand("turret-45", superstructure.setTurretRight());// Add more angles in super
                                                                                    // strucutre. search 60 Name ;
                                                                                    // turret-45
        ...
        NamedCommands.registerCommand("Auto Aim Red",
                turret.trackAprilTag(8, 9, 10, 11).until(() -> Math.abs(turret.getTagTx()) < 5.0).withTimeout(2));
        NamedCommands.registerCommand("Auto Aim Blue",
                turret.trackAprilTag(24, 25, 26, 27).until(() -> Math.abs(turret.getTagTx()) < 5.0).withTimeout(3));
        NamedCommands.registerCommand("Stop Intake", superstructure.intakeStop());
        autoChooser = AutoBuilder.buildAutoChooser();
        // NamedCommands.registerCommand("PivotDown",
        // superstructure.setIntakePivotAngle(Degrees.of(107))); //Registered pivot down
        // command
    ...
    private void configureBindings() {
        ...
        joystick.rightTrigger().onTrue(superstructure.intakeCommand());
        joystick.rightTrigger().onFalse(superstructure.intakeCommand());

        joystick.leftTrigger().onTrue(superstructure.shootCommand());
        joystick.leftTrigger().onFalse(superstructure.shootCommand());
        ...
        joystick.povDown().onTrue(superstructure.setIntakePivotAngle(Degrees.of(107)));
        joystick.povUp().onTrue(superstructure.setIntakePivotAngle(Degrees.of(1)));

        joystick.povLeft().onTrue(superstructure.setAngle90R()); // Set to -90
        joystick.povRight().onTrue(superstructure.setAngle90());
        ...
        */
        ...
        BCON.x().onTrue(superstructure.intakeCommand());
        BCON.x().onFalse(superstructure.intakeStop());

        BCON.leftBumper().onTrue(superstructure.shootCommand());
        BCON.leftBumper().onFalse(superstructure.stopShootCommand());

        BCON.leftTrigger().onTrue(superstructure.kickerFeedCommand().alongWith(superstructure.hopperFeedCommand()));
        BCON.leftTrigger().onFalse(superstructure.kickerStopCommand().alongWith(superstructure.hopper.stopCommand()));  
        
        BCON.a().onTrue(superstructure.setIntakePivotAngle(Degrees.of(107)));
        BCON.y().onTrue(superstructure.setIntakePivotAngle(Degrees.of(1)));
        
        BCON.b().onTrue(superstructure.ejectCommand());
        BCON.b().onFalse(superstructure.ejectCommand());

        BCON.povDown().onTrue(superstructure.backFeedAllCommand());
        BCON.povDown().onFalse(superstructure.backFeedAllCommand());

        BCON.rightBumper().onTrue(superstructure.setTurret90R());
        BCON.rightTrigger().onTrue(superstructure.setTurret90());

        BCON.povUp().onTrue(drivetrain.runOnce(drivetrain::seedFieldCentric));
        ...
        if (DriverStation.getAlliance().isPresent()) {
            if (DriverStation.getAlliance().get() == Alliance.Red) {
                joystick.leftBumper()
                        .onTrue(turret.trackAprilTag(8, 9, 10, 11).until(() -> Math.abs(turret.getTagTx()) < 5.0));
            }
            if (DriverStation.getAlliance().get() == Alliance.Blue) {
                joystick.leftBumper()
                        .onTrue(turret.trackAprilTag(24, 25, 26, 27).until(() -> Math.abs(turret.getTagTx()) < 5.0));
            }
        } else {
            joystick.leftBumper()
                    .onTrue(turret.trackAprilTag(8, 9, 10, 11).until(() -> Math.abs(turret.getTagTx()) < 5.0));
